...
class _ExactLoginScreenState extends State<ExactLoginScreen> {
  ...
  Future<void> _handleLogin() async {
    if (_email.trim().isEmpty || _password.isEmpty) { setState(() => _error = "Please fill in all fields"); return; }
    setState(() { _error = ''; _loading = true; });
    try {
      await Provider.of<AuthProvider>(context, listen: false).login(_email.trim(), _password);
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                            ...
                            child: Row(
                              children: [
                                  GlassCard(
                                    padding: EdgeInsets.zero,
                                    borderRadius: 4,
                                    fillAlpha: _rememberMe ? (t.isDark ? 0.3 : 0.8) : 0.0,
                                    tintColor: _rememberMe ? primaryBlue : null,
                                    child: SizedBox(
                                      width: 18,
                                      height: 18,
                                      child: _rememberMe ? Icon(Icons.check, size: 14, color: Colors.white) : null,
                                    ),
                                  ),
                                  const SizedBox(width: 8),
                                  Text('Remember me', style: TextStyle(fontSize: 13, color: placeholderColor, fontWeight: FontWeight.w500)),
                              ],
                            ),
                          ),
                          GestureDetector(
                            onTap: () {},
                            child: Text('Forget password?', style: TextStyle(fontSize: 13, color: primaryBlue, fontWeight: FontWeight.w500)),
                          ),
                        ],
                      ),
                      ...
                      // Or Sign in with
                      Center(child: Text('Or Sign in with', style: TextStyle(fontSize: 13, color: placeholderColor, fontWeight: FontWeight.w500))),
                      const SizedBox(height: 20),
                            ...
                            child: _googleLoading
                                ...
                                : Row(
                                    ...
                                    children: [
                                      ...
                                      Text('Continue with Google', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: textColor)),
                                    ],
                                  ),
                          ),
                        ),
                      ...
                      GestureDetector(
                        ...
                        child: Row(
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                            Text("Don't have an account? ", style: TextStyle(fontSize: 14, color: subtleText)),
                            Text("Create one", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: primaryBlue)),
                          ],
                        ),
                      ),
                    ],
